    ...
    FXPacker(p, opts, x, y, w, h)
{
    ...
    strlcpy_(selected.face, fontname.before(',').text(), sizeof(selected.face) + 1);
    selected.size = 90;
...
void FontSelector::listFontFaces()
{
    ...
    if (FXFont::listFonts(fonts, numfonts, FXString::null, 0, 0, selected.setwidth, selected.encoding, selected.flags))
    {
        ...
        if (0 < familylist->getNumItems())
        {
            ...
            strlcpy_(selected.face, familylist->getItemText(selindex).text(), sizeof(selected.face) + 1);
        }
        FXFREE(&fonts);
    }
}
...
{
    strlcpy_(selected.face, familylist->getItemText((int)(FXival)ptr).text(), sizeof(selected.face) + 1);
