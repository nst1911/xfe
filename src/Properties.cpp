...
PropertiesBox::PropertiesBox(FXWindow* win, FXString file, FXString path) : DialogBox(win, _("Properties"), DECOR_TITLE | DECOR_BORDER | DECOR_MAXIMIZE | DECOR_STRETCHABLE | DECOR_CLOSE)
{
    ...
    if (isDirectory)
    {
            ...
            while ((mnt = getmntent(mtab)))
            {
                if (!streq(mnt->mnt_type, MNTTYPE_IGNORE) && !streq(mnt->mnt_type, MNTTYPE_SWAP))
                {
                    if (streq(mnt->mnt_dir, dirpath.text()))
                    {
                        ...
                        type = buf;
                        strlcpy_(mnttype, mnt->mnt_type, strlen(mnt->mnt_type) + 1);
                    }
                }
            }
            endmntent(mtab);
        ...
        if (isMountpoint)
        {
            ...
            if (success)
            {
                ...
                char* pstr;
                pstr = strtok(NULL, " ");
                strlcpy_(used, pstr, strlen(pstr) + 1);  // get used
                pstr = strtok(NULL, " ");
                strlcpy_(avail, pstr, strlen(pstr) + 1); // get available
                pstr = strtok(NULL, " ");
                strlcpy_(pctr, pstr, strlen(pstr) + 1);  // get percentage
            }
            else
            {
                strlcpy_(used, "", 1);
    ...
    else
    {
        ...
        if ((extension2 == "tar.gz") || (extension2 == "tar.bz2") || (extension2 == "tar.xz") || (extension2 == "tar.zst") || (extension2 == "tar.z"))
        {
    ...
    if (cond)
    {
        ...
        if (ext->getText() != "")
        {
            ...
            if ((oldfileassoc == "") || (fileassoc.section('=', 1) != oldfileassoc))
            {
                ...
                strlcpy_(str[0], ext->getText().text(), ext->getText().length() + 1);
                strlcpy_(str[1], command.text(), command.length() + 1);
    ...
    if (recsize)
    {
        ...
        if (num == 1)
        {
            ...
            if (isDirectory && !isMountpoint)
            {
                ...
                strlcpy_(buf, dirpath.text(), dirpath.length() + 1);
        ...
        else
        {
            ...
            if (pid == 0)             // Child
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    if (isDirectory)
                    {
                        strlcpy_(buf, pathname.text(), pathname.length() + 1);
