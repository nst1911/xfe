...
long SearchPanel::onCmdOpenWith(FXObject*, FXSelector, void*)
{
    ...
    if (opendialog->execute())
    {
        ...
        for (int u = 0; u < list->getNumItems(); u++)
        {
            if (list->isItemSelected(u))
            {
                ...
                if (opendialog->getOption())
                {
                    ...
                    if ((ext == "tar.gz") || (ext == "tar.bz2") || (ext == "tar.xz") || (ext == "tar.zst") || (ext == "tar.z")) // Special cases
                    {}
                    else
                    {
                    ...
                    if (association)
                    {
                        ...
                        strlcpy_(str[0], ext.text(), ext.length() + 1);
                        strlcpy_(str[1], oldfileassoc.text(), oldfileassoc.length() + 1);
                    ...
                    else
                    {
                        ...
                        strlcpy_(str[0], ext.text(), ext.length() + 1);
                        strlcpy_(str[1], newcmd.text(), newcmd.length() + 1);
                        mainWindow->handle(this, FXSEL(SEL_COMMAND, XFileExplorer::ID_FILE_ASSOC), str);
                    }
                }
                // End
        ...
        {
            // FIFO
            strlcpy_(OpenHistory[0], cmd.text(), cmd.length() + 1);
            for (int i = 1; i < OpenNum; i++)
            {
                strlcpy_(OpenHistory[i], opendialog->getHistoryItem(i - 1).text(), opendialog->getHistoryItem(i - 1).length() + 1);
            }
        }
...
long SearchPanel::onCmdPopupMenu(FXObject* o, FXSelector s, void* p)
{
    ...
    else
    {
        ...
        if (list->getItem(itm) && list->isItemFile(itm))
        {
            ...
            if ((num == 1) && ((ext2 == "tar.gz") || (ext2 == "tar.bz2") || (ext2 == "tar.xz") || (ext2 == "tar.zst") || (ext2 == "tar.z")))
            {
                ar = true;
                new FXMenuCommand(menu, _("E&xtract to..."), archexticon, this, SearchPanel::ID_EXTRACT);
            }
            else if ((num == 1) && ((ext1 == "gz") || (ext1 == "bz2") || (ext1 == "zst") || (ext1 == "xz") || (ext1 == "z")))
            {
                ar = true;
                new FXMenuCommand(menu, _("&Extract here"), archexticon, this, SearchPanel::ID_EXTRACT);
                     ...
                     || (ext1 == "tzst") || (ext1 == "taz") || (ext1 == "txz") || (ext1 == "zip") || (ext1 == "7z")
...
long SearchPanel::onCmdAddToArch(FXObject* o, FXSelector, void*)
{
    ...
    if (archdialog->execute())
    {
        ...
        else if ((ext1 == "tgz") || (ext1 == "tbz2") || (ext1 == "tbz") || (ext1 == "txz") || (ext1 == "taz") || (ext1 == "tzst"))
        {
            cmd = "tar -acvf " + archive + " ";
        }
        else if (ext1 == "tar")
...
long SearchPanel::onUpdFileDelete(FXObject* o, FXSelector sel, void*)
{
    FXbool use_trash_can = getApp()->reg().readUnsignedEntry("OPTIONS", "use_trash_can", true);
...
long SearchPanel::onUpdFileTrash(FXObject* o, FXSelector sel, void*)
{
