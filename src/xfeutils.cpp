...
size_t strlcpy_(char* dst, const char* src, size_t sz)
{
...
// The destination string is always null terminated
size_t strlcat_(char* dst, const char* src, size_t sz)
{
    char* start = dst;
    ...
    dst--;
    return(dst - start + strlcpy_(dst, src, sz));
}
...
FXulong pathsize(char* path, FXuint* nbfiles, FXuint* nbsubdirs, FXulong *totalsize, int pipes[2])
{
    ...
    while ((dirp = readdir(dp)))
    {
        ...
        strlcpy_(ptr, dirp->d_name, strlen(dirp->d_name) + 1);
...
FXString hSize(char* size)
{
    ...
    strlcpy_(suf, _("bytes"), sizeof(suf));
    if (lsize > 1e9)
    {
        fsize = lsize / 1e9;
        strlcpy_(suf, _("GB"), sizeof(suf));
        flag = 1;
    }
    else if (lsize > 1e6)
    {
        fsize = lsize / 1e6;
        strlcpy_(suf, _("MB"), sizeof(suf));
        flag = 1;
    }
    else if (lsize > 1e3)
    {
        fsize = lsize / 1e3;
        strlcpy_(suf, _("kB"), sizeof(suf));
        flag = 1;
    }
...
int runst(FXString cmd)
{
    ...
    while (1)
    {
        ...
        if (str1[0] == '\'')
        {
            ...
            strlcpy_(args[nbargs], str2.text(), str2.length() + 1);
            nbargs++;
        }
        else
        {
            args[nbargs] = (char*)malloc(str1.length() + 1);
            strlcpy_(args[nbargs], str1.text(), str1.length() + 1);
            nbargs++;
        }
